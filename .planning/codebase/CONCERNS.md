...
When `refreshWorkosToken()` fails in `chat.headers`, the error is logged but the request continues with the expired token. The request will fail with a 401, and OpenCode will show a generic "authentication expired" message. The error classification hook catches this and shows a friendly message, but the user experience could be improved by proactively prompting re-authentication when refresh fails.
...
**Severity**: Minor | **File**: `src/auth.ts`, `src/clinepass.ts`
...
### 7. `as Record<string, any> as any` Type Casts
**Severity**: Minor | **File**: `src/clinepass.ts`

The `provider.models` hook returns `remote as Record<string, any> as any` to bridge between `ModelConfigEntry` and OpenCode's expected model type. This is a type escape hatch — ideally OpenCode's SDK would accept our typed model config directly.

### 8. No Rate Limit or Circuit Breaker for WorkOS Refresh
**Severity**: Minor | **File**: `src/workos.ts`
...
**Severity**: Info | **File**: `src/auth.ts`
...
**Severity**: Resolved | **File**: `src/models.ts`
...
**Severity**: Negligible | **File**: `src/env.ts`
